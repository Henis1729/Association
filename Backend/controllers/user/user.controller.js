...
const helpers = {
  ...
  generateSearchQuery: async ({ req }) => {
    ...
    if (req.query.role) {
      const { _id } =
        (await DB.ROLE.findOne({ name: req.query.role }).lean()) || {};
...
const controllers = {
  ...
  sendOTP: async (req, res) => {
    ...
    if (otpExists) {
      ...
      otpExists.expiryDate = new Date(
        Date.now() + req.body.expiryInMinutes * 60 * 1000
      );
  ...
  verifyOTP: async (req, res) => {
    ...
    if (!otp) return response.BAD_REQUEST({ res, message: MESSAGE.NOT_FOUND });
  ...
  fetch: async (req, res) => {
    ...
    if (!isValidFilter.isSuccess)
      return response.UNAUTHORIZED({
        ...
        payload: { context: isValidFilter.message },
      });
